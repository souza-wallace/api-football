<template>
  <div>
      ...
      <div class="card p-4 shadow-sm">
        <div class="row g-3 align-items-end">
          ...
          <div class="col-md-3">
            ...
            <select class="form-select" v-model="country" @change="getTeams">
          ...
          <!-- Campo de busca -->
          <div class="col-md-6">
            <input class="form-control" type="search" placeholder="Busque por um time" aria-label="Busque por um time" v-model="searchQuery" @input="searchTeam" :disabled="!country">
          </div>
          <!-- Botão Pesquisar -->
    ...
    <div class="container mt-5">
      <h5 class="mb-4">Times</h5>
      <div class="row">
        <div class="col-md-3 mb-4" v-for="(t, index) in paginatedTeams" :key="index">
          <router-link :to="`/team/${t.team.id}`" class="border">
            <div class="card d-flex flex-column align-items-center p-3">
              <div class="d-flex justify-content-center w-100">
                <img :src="t.team.logo" class="card-img-top" alt="league Emblem" style="height: 100px; width: auto; object-fit: contain;">
              </div>
              <div class="card-body text-center">
                <h5 class="card-title">{{ t.team.name }}</h5>
              </div>
            </div>
          </router-link> 
        </div>
      </div>
      <div class="text-center mt-3" v-if="paginatedTeams.length < teams.length">
...
export default {
  ...
  computed: {
    paginatedTeams() {
      return this.filteredTeams.slice(0, this.visibleCount);
    },
    filteredTeams() {
      return this.teams.filter(t => t.team.name.toLowerCase().includes(this.searchQuery.toLowerCase()));
    }
  },
  ...
  methods: {
    ...
    async getTeams(){
		const response = await fetch(`http://127.0.0.1:8000/teams/country/${this.country}`, {
		method: 'GET',
		headers: {
